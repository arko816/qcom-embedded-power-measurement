...
param(
    ...
    [string]$QwtArchivePath = $env:QEPM_QWT_ARCHIVE,
...
if ($policy.product -ne "QEPM" -or
    ...
    $policy.configurationPattern -ne "*.ccnf") {
...
if (-not $SkipBuild) {
    ...
    if ([string]::IsNullOrWhiteSpace($QtRoot)) {
        throw "Set QEPM_QT_ROOT or pass -QtRoot, for example C:\Qt\6.9.3\msvc2022_64."
    }
...
elseif (-not [string]::IsNullOrWhiteSpace($QwtArchivePath)) {
    if (-not (Test-Path -Path $QwtArchivePath -PathType Leaf)) {
        throw "QwtArchivePath does not exist: $QwtArchivePath"
    }

    $QwtArchivePath = (Resolve-Path $QwtArchivePath).Path
}
...
if (-not $SkipBuild) {
    ...
    if (-not [string]::IsNullOrWhiteSpace($QwtArchivePath)) {
        $configureArguments += "-DQEPM_QWT_ARCHIVE=$QwtArchivePath"
    }

    Invoke-Process `
        -FilePath $CMakePath `
        -Arguments $configureArguments `
        -Description "QEPM CMake configure"
    ...
    Invoke-Process `
        ...
        -Description "QEPM Release build"
}

if (-not (Test-Path -Path $runtimeSource -PathType Container)) {
    throw "QEPM Release runtime output is missing: $runtimeSource. The project controls this output through Common.cmake."
}
...
if ($missing.Count -gt 0) {
    throw "QEPM Release output is incomplete. Missing: $($missing -join ', ')"
}
...
if ($configurations.Count -ne $expectedConfigurationCount) {
    throw "QEPM configuration count mismatch. Expected $expectedConfigurationCount, found $($configurations.Count)."
}
...
$manifest = [PSCustomObject]@{
    ...
    qwtArchiveFile = if ([string]::IsNullOrWhiteSpace($QwtArchivePath)) {
        $null
    }
    else {
        Split-Path -Leaf $QwtArchivePath
    }
    qwtArchiveSha256 = if ([string]::IsNullOrWhiteSpace($QwtArchivePath)) {
        $null
    }
    else {
        (Get-FileHash -Path $QwtArchivePath -Algorithm SHA256).Hash
    }
    files = $files
}
